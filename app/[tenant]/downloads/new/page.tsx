...
export default async function NewDownloadPage({ params }: { params: { tenant: string } }) {
  const { tenant } = await Promise.resolve(params)
  async function handleCreate(formData: FormData) {
    'use server'
    await createDownload(tenant, formData)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-semibold text-[rgb(var(--text-1))] mt-2">Add Download</h1>
      </div>
      <form action={handleCreate} className="space-y-5">
        ...
        <div>
          ...
          <select name="category" defaultValue="other" className="w-full px-3 py-2 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--surface))] text-[rgb(var(--text-1))] text-sm focus:outline-none focus:ring-2 focus:ring-[rgb(var(--accent))]">
        ...
        <div>
          ...
          <input name="file_url" type="url" required placeholder="https://" className="w-full px-3 py-2 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--surface))] text-[rgb(var(--text-1))] text-sm focus:outline-none focus:ring-2 focus:ring-[rgb(var(--accent))]" />
        </div>
        <div>
          <label className="block text-sm font-medium text-[rgb(var(--text-1))] mb-1">File Name *</label>
          <input name="file_name" required placeholder="report-q4-2024.pdf" className="w-full px-3 py-2 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--surface))] text-[rgb(var(--text-1))] text-sm focus:outline-none focus:ring-2 focus:ring-[rgb(var(--accent))]" />
        </div>
        <div>
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" className="px-5 py-2 rounded-lg bg-[rgb(var(--accent))] text-white text-sm font-medium hover:opacity-90 transition-opacity">Add Download</button>
